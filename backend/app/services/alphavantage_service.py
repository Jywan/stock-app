...
class AlphaVantageService:
    ...
    def weekly_price(symbol: str):
        ...
        if "Weekly Time Series" not in data:
            raise ApiError("주간 데이터 없음", 400, data)

        return data, url
    
    @staticmethod
    def monthly_price(symbol: str):
        api_key = current_app.config['ALPHAVANTAGE_API_KEY']
        ...
        url = (
            "https://www.alphavantage.co/query"
            f"?function=TIME_SERIES_MONTHLY&symbol={symbol}"
            f"&apikey={api_key}"
        )
        ...
        if "Monthly Time Series" not in data:
            raise ApiError("월간 데이터 없음", 400, data)
    ...
    def news_sentiment(symbol: str):
        ...
        # AlphaVantage NEWS_SENTIMENT 엔드포인트
        url = (
            "https://www.alphavantage.co/query"
        ...
        # NEWS_SENTIMENT 응답 구조상 "feed" 필드에 뉴스 목록이 들어옴
        if "feed" not in data:
            raise ApiError("뉴스 데이터 없음", 400, data)
